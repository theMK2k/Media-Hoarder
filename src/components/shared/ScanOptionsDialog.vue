    ...
    <v-card dark flat v-bind:ripple="false">
      ...
      <v-card-text>
        ...
        <v-radio-group v-model="radioGroup">
          <v-radio
            v-bind:label="$t('Quick Scan')"
            v-bind:value="1"
            color="mk-dark-grey"
          ></v-radio>
          <v-radio
            v-bind:label="$t('Complete Rescan')"
            v-bind:value="2"
            color="mk-dark-grey"
          ></v-radio>
        </v-radio-group>
        ...
        <div v-if="radioGroup == 1">
          <ul>
            ...
            <li>
              <strong>{{
                $t("If a new file is found, gather IMDB and mediainfo metadata")
              }}</strong>
            </li>
            <li>
              <strong>{{
        ...
        <div v-if="radioGroup == 2">
          <ul>
            ...
            <li>
              <strong>{{
                $t(
                  "If a file is new or already known, gather IMDB and mediainfo metadata"
                )
              }}</strong>
            </li>
            <li>
              <strong>{{
                $t(
                  "This can take a while depending on your collection and internet connection"
                )
              }}</strong>
            </li>
          </ul>
        ...
        <v-expansion-panels accordion style="margin-top: 16px">
          <v-expansion-panel style="padding: 0px !important">
            <v-expansion-panel-header style="padding: 8px !important"
              >{{ $t("IMDB Scraper Options") }}
              {{ imdbOptionsTitle }}</v-expansion-panel-header
            >
        ...
        >
          ...
          <ul>
            <li
              v-for="msp in missingSourcePaths"
              v-bind:key="msp.id_SourcePaths"
            >
              {{ msp.Path }}
            </li>
          </ul>
        ...
        <v-col sm="12">
          <v-row style="margin-top: 8px">
            ...
            <v-btn
              class="xs-fullwidth"
              v-bind:color="
                missingSourcePaths && missingSourcePaths.length > 0
                  ? 'red'
                  : 'primary'
              "
