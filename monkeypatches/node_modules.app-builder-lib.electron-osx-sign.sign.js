...
function validateSignOptsAsync(opts) {
  ...
  if (opts["provisioning-profile"]) {
    if (typeof opts["provisioning-profile"] !== "string" && !(opts["provisioning-profile"] instanceof ProvisioningProfile))
      return Promise.reject(new Error("Path to provisioning profile should be a string or a ProvisioningProfile object."));
  }

  if (opts["type"]) {
    if (opts["type"] !== "development" && opts["type"] !== "distribution")
      return Promise.reject(new Error("Type must be either `development` or `distribution`."));
  } else {
    opts["type"] = "distribution";
  }

  return Promise.all([validateOptsAppAsync(opts), validateOptsPlatformAsync(opts), validateOptsBinariesAsync(opts)]);
}
...
function verifySignApplicationAsync(opts) {
  ...
  let promise = execFileAsync(
    "codesign",
    ["--verify", "--deep"].concat(
      opts["strict-verify"] !== false && semver.gte(osRelease, "15.0.0") >= 0 // Only pass strict flag in El Capitan and later
        ? [
            "--strict" +
              (opts["strict-verify"]
  ...
  if (opts.platform === "darwin" && opts["gatekeeper-assess"] !== false) {
    promise = promise.then(function () {
      ...
      return execFileAsync("spctl", ["--assess", "--type", "execute", "--verbose", "--ignore-cache", "--no-cache", opts.app]);
    });
  }

  return promise.thenReturn();
}
...
function signApplicationAsync(opts) {
  return walkAsync(getAppContentsPath(opts)).then(async function (childPaths) {
    /**
     * MONKEYPATCHED!
     * https://github.com/electron-userland/electron-builder/issues/4969
    ...
    const args = ["--sign", opts.identity.hash || opts.identity.name, "--force"];
    if (opts.keychain) {
...
const signAsync = (module.exports.signAsync = function (opts) {
  return validateSignOptsAsync(opts)
    .then(function () {
      ...
      return promise.then(function (identities) {
        ...
        if (!opts.entitlements) {
          ...
          if (opts.entitlements === true) {
            ...
            debugwarn(
              ...
              "* Provide `entitlements` to specify entitlements file for codesign.",
              "\n",
              "* Sandbox entitlements file for enclosing app files is default to:",
              filePath
            );
            opts.entitlements = filePath;
          }
